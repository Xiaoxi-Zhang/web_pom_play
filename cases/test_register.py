...
class TestRegister:
    ...
    def test_register_6(self):
        """密码框不能有特殊字符"""
        self.register.fill_password('abc12!@#')
        # 断言
    ...
    def test_register_error(self):
        """测试注册正常流程， 已被注册过的账号"""
        self.register.fill_username('test')
        self.register.fill_password('123456')
        self.register.click_register_button()
        # 断言提示语可见
        expect(self.register.locator_register_error).to_be_visible()  # 用户名已存在

    def test_register_success(self):
        """测试注册正常流程， 注册新账号成功"""
        self.register.fill_username(str(uuid.uuid4())[:8])  # 随机生成用户名
        self.register.fill_password('123456')
        self.register.click_register_button()
        # 断言页面跳转到登录页
